...
function page() {
  return (
    <>
        <div className='w-full h-[300px] flex flex-col text-center items-center justify-center px-4'>
            <h1 className='text-4xl font-[300]'>Articles</h1>
            <p className='text-lg font-[300] mt-5 text-gray-700'>Welcome to the Articles page. Here you can find a collection of articles on various topics.</p>
        </div>
        <div className='max-w-5xl mx-auto p-4 grid grid-cols-1 md:grid-cols-2 gap-6 pb-32'>
            {articlesData.map((article) => (
                <Link href={`/articles/${article.slug}`} key={article.id} className='bg-white border rounded-lg p-6 hover:shadow-lg transition-shadow duration-200'>
                    <div className='mb-3'>
